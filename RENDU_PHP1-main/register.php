...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
       ...
       if (empty($errors)) {
        ...
        $escapedMdp = $conn->quote($mdp);

        $hashedPassword = password_hash($escapedMdp, PASSWORD_BCRYPT);

        // Construire la requête SQL
        $sql = "INSERT INTO users (nom, prenom, nom_utilisateur, mdp) VALUES ($escapedNom, $escapedPrenom, $escapedLogin, '$hashedPassword')";
